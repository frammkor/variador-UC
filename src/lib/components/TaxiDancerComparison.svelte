...
<style>
  .comparison { color: var(--color-text); background: var(--color-background); }
  ...
  .comparison :global(.eyebrow) { color: var(--color-primary); }
  ...
  .comparison-row { display: grid; min-width: 0; box-sizing: border-box; flex: 0 0 calc(100% - 1.5rem); grid-template-columns: repeat(2, minmax(0, 1fr)); overflow: hidden; border: 1px solid var(--color-primary-200); border-radius: 1rem; background: #fff; scroll-snap-align: start; scroll-snap-stop: always; }
  .comparison-row h3 { grid-column: 1 / -1; margin: 0; padding: 1rem; color: var(--color-heading); font-size: 1rem; font-weight: 600; text-align: center; }
  .comparison-row div { display: flex; min-height: 6.5rem; padding: .85rem 1rem; align-items: center; justify-content: center; flex-direction: column; border-top: 1px solid var(--color-primary-100); text-align: center; }
  .comparison-row div:nth-of-type(2) { border-left: 1px solid var(--color-primary-100); }
  .comparison-row .dandy { grid-column: 1 / -1; min-height: 5.5rem; border-left: 0; background: var(--color-primary-50); }
  ...
  @media (min-width: 64rem) {
    ...
    .comparison-head strong { padding: .85rem 1rem; color: var(--color-muted); font-size: .8rem; }
    .comparison-head .dandy-heading { border-radius: .75rem .75rem 0 0; color: var(--color-primary-900); background: var(--color-primary-50); }
