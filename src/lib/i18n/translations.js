export const translations = {
  en: {
    navigation: {
      languageSelector: 'Language selector',
      tagline: 'Spend less time sitting and more time dancing.',
      socialLinks: 'Social media and contact links',
      whatsappMessage: 'Hi, I am interested in the taxi dancer service.'
    },
    metaTitle: 'Tango Dance Partners in Buenos Aires | Dandy Dancers',
    metaDescription:
      'Book a professional tango dance partner for milongas, lessons, and practice in Buenos Aires. A welcoming local tango experience for adult travelers.',
    hero: {
      title: 'Book Taxi Dancers',
      supporting:
        'We help you find a dancer for the tango experience you dream of in Buenos Aires.',
      cta: 'I want a taxi dancer',
      registrationCta: 'I want to be a Dandy',
      preview: {
        label: 'Illustrative platform preview',
        tapHint: 'Swipe to explore',
        progressLabel: 'Preview steps',
        steps: ['Select your milonga', 'Pick an available dancer', 'Reservation confirmed'],
        eventType: 'Milonga',
        viewProfile: 'View profile',
        age: 'Age',
        reviews: 'Reviews',
        candidatesPending: 'More candidates may still apply…',
        continue: 'Continue',
        successLead: 'Your tango night is all set.',
        successNote: 'We’ll send you all the details.',
        withDancer: 'with Tito'
      }
    },
    taxiDancer: {
      title: 'What is a taxi dancer?',
      modalClose: 'Close',
      description:
        'A taxi dancer is a professional dancer you can book to share a milonga, class, or practice. Beyond dancing, they can help you feel comfortable in the local scene, understand its customs, and make the most of your time on the dance floor.'
    },
    previewSection: {
      steps: [
        {
          title: 'Meet your Dandy in 3 simple steps',
          description: 'See how easy it is.'
        },
        {
          title: 'Choose the right dancer for your night',
          description: 'Explore the available dancers and choose the Dandy you want.'
        },
        {
          title: 'Your night, guaranteed',
          description: 'You’re in good hands. Just enjoy the milonga.'
        }
      ]
    },
    why: {
      eyebrow: 'Why Dandy Dancers',
      title: 'Never sit out a dance',
      intro:
        'Your time in Buenos Aires is valuable. A trusted dancer turns uncertainty into the dance experience you were looking for.',
      cta: 'I want a taxi dancer',
      items: [
        {
          title: 'Feel part of the dance scene',
          description:
            'Arrive with someone who helps you understand the local codes and feel welcome from the first tanda.'
        },
        {
          title: 'Dance at the level you want',
          description:
            'Choose a dancer who matches your style and makes every tanda rewarding, whatever your level.'
        },
        {
          title: 'Get more from classes',
          description:
            'Share a lesson with a reliable dancer who can also help translate the teacher’s instructions.'
        },
        {
          title: 'Connect with the local community',
          description:
            'Get to know the scene and connect with the local community without the discomfort of arriving alone.'
        },
        {
          title: 'Practice in a real setting',
          description:
            'Try new techniques, ask questions, and build confidence without feeling judged.'
        },
        {
          title: 'Enjoy comfort and peace of mind',
          description:
            'Enjoy reliable company and personalized guidance while making the most of your time in Buenos Aires.'
        }
      ]
    },
    dancers: {
      eyebrow: 'Our Dandies',
      title: 'Meet the dancers',
      intro: 'Every embrace holds a story. Choose who you want to share yours with.',
      carouselLabel: 'Dandy Dancers profiles',
      rating: 'Rating',
      primaryCta: 'Very soon',
      registrationCta: 'I want to be a Dandy'
    },
    about: {
      eyebrow: 'Behind the project',
      title: 'About the founder',
      description:
        'Dandy Dancers is an independent project founded to make tango experiences in Buenos Aires easier to access, more comfortable, and more memorable.',
      moreLink: 'More about Franco',
      modalTitle: 'Meet Franco Cespi',
      modalDescription:
        'Franco Cespi is the founder of Dandy Dancers. He created the project around a simple idea: helping more people experience tango in Buenos Aires with confidence and in good company.',
      websiteLink: 'francocespi.com',
      instagramLink: '@franco.cespi',
      modalClose: 'Close founder information'
    },
    registration: {
      eyebrow: 'Be part of our taxi dancer team',
      title: 'Join Dandy Dancers',
      metaTitle: 'Register as a Taxi Dancer — Dandy Dancers',
      metaDescription: 'Apply to join the Dandy Dancers team of taxi dancers.',
      paragraphs: [
        'Complete the form so we can get to know you a little better and evaluate your profile.',
        'If your profile matches what we are looking for, we will contact you to tell you about the next steps.'
      ],
      cta: 'I want to join!',
      why: {
        eyebrow: 'Why join Dandy Dancers',
        title: 'More opportunities, less chasing',
        intro: 'Find clear opportunities in one place and choose what fits your schedule.',
        cta: 'I want to join!',
        items: [
          { title: 'Find real opportunities', description: 'Meet people already looking for a taxi dancer.' },
          { title: 'Keep everything together', description: 'Stop chasing work across chats, groups, and social media.' },
          { title: 'Organize your schedule', description: 'See requests by day and time before committing.' },
          { title: 'Know who you’ll dance with', description: 'Review their profile, dance level, and plan for the night.' },
          { title: 'Show your experience', description: 'A professional profile gives your work more visibility.' },
          { title: 'Stay in control', description: 'Choose when, where, and with whom you want to dance.' }
        ]
      },
      previewSection: {
        steps: [
          { title: 'They’re looking for you', description: 'Explore taxi dancer requests ordered by day and time, then choose the one that fits your schedule.' },
          { title: 'Choose your next dance', description: 'Put your name forward to dance with the person you’re most excited to meet.' },
          { title: 'You’ve made your cabeceo', description: 'We’ll let you know if you’re selected.' }
        ]
      },
      preview: {
        label: 'Illustrative platform preview',
        progressLabel: 'Availability preview steps',
        tapHint: 'Swipe to explore',
        steps: ['Looking for a Dandy', 'Request details', 'You’ve made your cabeceo'],
        days: ['Thursday, October 24', 'Friday, October 25', 'Saturday, October 26'],
        availableSearches: 'Available taxi dancer requests',
        person: 'person',
        people: 'people',
        milonga: 'Milonga',
        date: 'Date',
        meetingTime: 'Meeting time',
        duration: 'Duration',
        dancingSince: 'Dancing since',
        favoriteOrchestra: 'Favorite orchestra',
        profile: { name: 'Sophie', level: 'Beginner', since: '2024', nationality: 'French', role: 'Follower', orchestra: 'Osvaldo Pugliese', bio: 'Discovering Buenos Aires tango with curiosity and joy.' },
        apply: 'I’m available',
        successLead: 'We let them know you’re available.',
        successNote: 'We’ll let you know if you’re selected.'
      },
      comparison: {
        eyebrow: 'A better way to work',
        title: 'Value your embrace',
        intro: 'Combine the control of working independently with the speed of an intermediary.',
        carouselLabel: 'Comparison topics',
        swipeHint: 'Swipe to see more',
        columns: { solo: 'On your own', intermediary: 'With an intermediary', dandy: 'With Dandy Dancers' },
        items: [
          { title: 'Your rate', solo: 'You negotiate it.', intermediary: 'They set it.', dandy: 'You set it.' },
          { title: 'Your time', solo: 'Coordinating everything takes time.', intermediary: 'You delegate, but lose control.', dandy: 'See what’s available and sign up in seconds.' },
          { title: 'Confirming the job', solo: 'Messages and manual follow-up.', intermediary: 'Back-and-forth through a third party.', dandy: 'We tell you when and where.' },
          { title: 'Who you dance with', solo: 'You depend on your network.', intermediary: 'You depend on others.', dandy: 'You choose.' }
        ]
      }
    },
    testimonials: {
      eyebrow: 'Testimonials',
      titleStart: 'More than a dance—',
      titleHighlight: 'a shared moment.',
      placeholderLabel: 'Sample testimonial',
      carouselLabel: 'Testimonial slides'
    },
    footer: {
      navigationTitle: 'Explore',
      navigationLabel: 'Main page sections',
      socialTitle: 'Follow us',
      whatsappMessage: 'Hi, I am interested in the taxi dancer service.'
    }
  },
  es: {
    navigation: {
      languageSelector: 'Selector de idioma',
      tagline: 'Pasá menos tiempo en la silla y más tiempo bailando.',
      socialLinks: 'Redes sociales y enlaces de contacto',
      whatsappMessage: 'Hola, me interesa el servicio de taxi dancers.'
    },
    metaTitle: 'Bailarines de Tango en Buenos Aires | Dandy Dancers',
    metaDescription:
      'Reservá un bailarín profesional de tango para milongas, clases y prácticas en Buenos Aires. Una experiencia local para viajeros adultos.',
    hero: {
      title: 'Contrata taxi dancers',
      supporting:
        'Te ayudamos a encontrar un bailarín para vivir la experiencia de tango que soñás en Buenos Aires.',
      cta: 'Quiero un taxi dancer',
      registrationCta: 'Quiero ser un Dandy',
      preview: {
        label: 'Vista ilustrativa de la plataforma',
        tapHint: 'Deslizá para explorar',
        progressLabel: 'Pasos de la demostración',
        steps: ['Elegí tu milonga', 'Elegí un bailarín disponible', 'Reserva confirmada'],
        eventType: 'Milonga',
        viewProfile: 'Ver perfil',
        age: 'Edad',
        reviews: 'Reseñas',
        candidatesPending: 'Todavía pueden sumarse más candidatos…',
        continue: 'Continuar',
        successLead: 'Tu noche de tango ya está lista.',
        successNote: 'Te enviaremos todos los detalles.',
        withDancer: 'con Tito'
      }
    },
    taxiDancer: {
      title: '¿Qué es un taxi dancer?',
      modalClose: 'Cerrar',
      description:
        'Un taxi dancer es un bailarín profesional que podés contratar para compartir una milonga, una clase o una práctica. Además de bailar, puede ayudarte a sentirte cómodo en la escena local, comprender sus códigos y aprovechar al máximo tu tiempo en la pista.'
    },
    previewSection: {
      steps: [
        {
          title: 'Conocé a tu Dandy en 3 simples pasos',
          description: 'Mirá lo fácil que es.'
        },
        {
          title: 'Elegí al bailarín ideal para tu noche',
          description: 'Explorá los bailarines disponibles y elegí al Dandy que quieras.'
        },
        {
          title: 'Tu noche asegurada',
          description: 'Estás en buenas manos. Vos disfrutá de la milonga.'
        }
      ]
    },
    why: {
      eyebrow: 'Por qué Dandy Dancers',
      title: 'No te quedes sin bailar',
      intro:
        'Tu tiempo en Buenos Aires es valioso. Un bailarín de confianza transforma la incertidumbre en la experiencia de baile que estabas buscando.',
      cta: 'Quiero un taxi dancer',
      items: [
        {
          title: 'Sentite parte de la escena de baile',
          description:
            'Llegá con alguien que te ayude a entender los códigos locales y sentirte parte desde la primera tanda.'
        },
        {
          title: 'Bailá al nivel que buscás',
          description:
            'Elegí un bailarín acorde a tu estilo para disfrutar cada tanda, sin importar tu nivel.'
        },
        {
          title: 'Aprovechá mejor las clases',
          description:
            'Compartí la clase con un bailarín confiable que también pueda ayudarte a traducir las indicaciones.'
        },
        {
          title: 'Conectate con la comunidad local',
          description:
            'Conocé el ambiente y conectate con la comunidad local sin la incomodidad de llegar solo.'
        },
        {
          title: 'Practicá en un entorno real',
          description:
            'Probá nuevas técnicas, hacé preguntas y ganá confianza sin sentirte juzgado.'
        },
        {
          title: 'Disfrutá con comodidad y tranquilidad',
          description:
            'Disfrutá de compañía confiable y orientación personalizada mientras aprovechás tu tiempo en Buenos Aires.'
        }
      ]
    },
    dancers: {
      eyebrow: 'Nuestros Dandies',
      title: 'Conocé a los bailarines',
      intro: 'En cada abrazo, una historia. Elegí con quién compartir la tuya.',
      carouselLabel: 'Perfiles de Dandy Dancers',
      rating: 'Puntuación',
      primaryCta: 'Muy pronto',
      registrationCta: 'Quiero ser un Dandy'
    },
    about: {
      eyebrow: 'Detrás del proyecto',
      title: 'Sobre el fundador',
      description:
        'Dandy Dancers es un proyecto independiente creado para hacer que las experiencias de tango en Buenos Aires sean más accesibles, cómodas y memorables.',
      moreLink: 'Más sobre Franco',
      modalTitle: 'Conocé a Franco Cespi',
      modalDescription:
        'Franco Cespi es el fundador de Dandy Dancers. Creó el proyecto a partir de una idea simple: ayudar a que más personas vivan el tango en Buenos Aires con confianza y en buena compañía.',
      websiteLink: 'francocespi.com',
      instagramLink: '@franco.cespi',
      modalClose: 'Cerrar información sobre el fundador'
    },
    registration: {
      eyebrow: 'Sé parte de nuestro equipo de taxi dancers',
      title: 'Sumate a Dandy Dancers',
      metaTitle: 'Registrate como taxi dancer — Dandy Dancers',
      metaDescription: 'Postulate para formar parte del plantel de taxi dancers de Dandy Dancers.',
      paragraphs: [
        'Completá el formulario para que podamos conocerte un poco más y evaluar tu perfil.',
        'Si tu perfil encaja con lo que estamos buscando, nos vamos a poner en contacto con vos para contarte los próximos pasos.'
      ],
      cta: '¡Quiero sumarme!',
      why: {
        eyebrow: 'Por qué sumarte',
        title: 'Más oportunidades, menos vueltas',
        intro: 'Encontrá propuestas claras en un solo lugar y elegí las que encajen con tu agenda.',
        cta: '¡Quiero sumarme!',
        items: [
          { title: 'Encontrá oportunidades reales', description: 'Conocé personas que ya buscan un taxi dancer.' },
          { title: 'Tené todo en un solo lugar', description: 'Dejá de perseguir trabajos entre chats, grupos y redes.' },
          { title: 'Ordená tu agenda', description: 'Mirá las búsquedas por día y hora antes de comprometerte.' },
          { title: 'Sabé con quién vas a bailar', description: 'Revisá su perfil, nivel y plan para la noche.' },
          { title: 'Mostrá tu experiencia', description: 'Un perfil profesional le da más visibilidad a tu trabajo.' },
          { title: 'Elegí con libertad', description: 'Decidí cuándo, dónde y con quién querés bailar.' }
        ]
      },
      previewSection: {
        steps: [
          { title: 'Te están buscando', description: 'Explorá búsquedas de taxi dancers ordenadas por día y hora y elegí la que mejor encaje con tu agenda.' },
          { title: 'Elegí tu trabajo', description: 'Anotate para bailar con quien más te entusiasme.' },
          { title: 'Ya hiciste tu cabeceo', description: 'Te avisaremos si fuiste elegido.' }
        ]
      },
      preview: {
        label: 'Vista ilustrativa de la plataforma',
        progressLabel: 'Pasos de la demostración de disponibilidad',
        tapHint: 'Deslizá para explorar',
        steps: ['Se busca Dandy', 'Detalle de la búsqueda', 'Ya hiciste tu cabeceo'],
        days: ['Jueves 24 de octubre', 'Viernes 25 de octubre', 'Sábado 26 de octubre'],
        availableSearches: 'Búsquedas disponibles de taxi dancers',
        person: 'persona',
        people: 'personas',
        milonga: 'Milonga',
        date: 'Fecha',
        meetingTime: 'Encuentro',
        duration: 'Duración',
        dancingSince: 'Baila desde',
        favoriteOrchestra: 'Orquesta favorita',
        profile: { name: 'Sophie', level: 'Principiante', since: '2024', nationality: 'Francesa', role: 'Seguidora', orchestra: 'Osvaldo Pugliese', bio: 'Descubriendo el tango de Buenos Aires con curiosidad y alegría.' },
        apply: 'Estoy disponible',
        successLead: 'Le avisamos que estás disponible.',
        successNote: 'Te avisaremos si fuiste elegido.'
      },
      comparison: {
        eyebrow: 'Otra forma de trabajar',
        title: 'Tu abrazo vale',
        intro: 'Combiná el control de trabajar por tu cuenta con la agilidad de un intermediario.',
        carouselLabel: 'Temas de comparación',
        swipeHint: 'Deslizá para ver más',
        columns: { solo: 'Por tu cuenta', intermediary: 'Con intermediario', dandy: 'Con Dandy Dancers' },
        items: [
          { title: 'Tu tarifa', solo: 'La negociás.', intermediary: 'La definen.', dandy: 'La definís vos.' },
          { title: 'Tu tiempo', solo: 'Coordinar todo lleva tiempo.', intermediary: 'Delegás, pero perdés el control.', dandy: 'Ves qué hay y te anotás en segundos.' },
          { title: 'Cerrar el trabajo', solo: 'Mensajes y seguimiento manual.', intermediary: 'Idas y vueltas con un tercero.', dandy: 'Te avisamos cuándo y dónde.' },
          { title: 'Con quién bailás', solo: 'Dependés de tu red.', intermediary: 'Dependés de otros.', dandy: 'Vos elegís.' }
        ]
      }
    },
    testimonials: {
      eyebrow: 'Testimonios',
      titleStart: 'Más que un baile:',
      titleHighlight: 'un momento compartido.',
      placeholderLabel: 'Testimonio de muestra',
      carouselLabel: 'Carrusel de testimonios'
    },
    footer: {
      navigationTitle: 'Explorá',
      navigationLabel: 'Secciones de la página principal',
      socialTitle: 'Seguinos',
      whatsappMessage: 'Hola, me interesa el servicio de taxi dancers.'
    }
  }
};
